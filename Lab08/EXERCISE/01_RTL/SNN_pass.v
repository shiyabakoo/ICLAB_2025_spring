...
GATED_OR gate_u1(.CLOCK(clk), .SLEEP_CTRL(cg_en && !state_enable), .RST_N(rst_n), .CLOCK_GATED(g_clk_state));
GATED_OR gate_cnn1(.CLOCK(clk), .SLEEP_CTRL(cg_en && !(c_state == CNN_1)), .RST_N(rst_n), .CLOCK_GATED(g_clk_cnn1));
GATED_OR gate_cnn2(.CLOCK(clk), .SLEEP_CTRL(cg_en && !(c_state == CNN_2)), .RST_N(rst_n), .CLOCK_GATED(g_clk_cnn2));
generate
	for(j = 0;j < 36;j = j + 1) begin
		GATED_OR gate_input(.CLOCK(clk), .SLEEP_CTRL(cg_en && !(in_cnt == j)), .RST_N(rst_n), .CLOCK_GATED(g_clk_input[j]));
	end
endgenerate
...
always @(posedge g_clk_state or negedge rst_n) begin
	if (!rst_n)
		state_cnt <= 0;
...
//--------------------------------------------------
// quantization
always @(posedge g_clk_state) begin
	if ((c_state == CNN_1) || (c_state == CNN_2)) begin
...
wire	g_clk_fully;
GATED_OR gate_fully(.CLOCK(clk), .SLEEP_CTRL(cg_en && !(state_enable && ((state_cnt == 8) | (state_cnt == 16)))), .RST_N(rst_n), .CLOCK_GATED(g_clk_fully));
...
wire	quan_clk1;
wire	quan_clk2;
GATED_OR gate_quan_1(.CLOCK(clk), .SLEEP_CTRL(cg_en && !((c_state == CNN_1) && (state_cnt == 'd9))), .RST_N(rst_n), .CLOCK_GATED(quan_clk1));
GATED_OR gate_quan_2(.CLOCK(clk), .SLEEP_CTRL(cg_en && !((c_state == CNN_1) && (state_cnt == 'd17))), .RST_N(rst_n), .CLOCK_GATED(quan_clk2));


always @(posedge quan_clk1) begin
...
always @(posedge quan_clk2) begin
	if ((c_state == CNN_1)) begin
...
// 	end
// end
always @(posedge g_clk_cnn2) begin 
